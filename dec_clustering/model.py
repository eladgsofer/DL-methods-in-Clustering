...
from dec_clustering.utils import target_distribution, cluster_accuracy
...
def train(
    ...
    :return: None
    """
    static_dataloader = DataLoader(dataset, batch_size=batch_size, collate_fn=collate_fn,
                                   pin_memory=False, sampler=sampler, shuffle=False, )
    ...
    data_iterator = tqdm(static_dataloader, leave=True, unit="batch",
                         postfix={"epo": -1, "acc": "%.4f" % 0.0, "lss": "%.8f" % 0.0,
                                  "dlb": "%.4f" % -1, }, disable=silent, )
    ...
    cluster_loss_func = nn.KLDivLoss(size_average=False)
    struct_loss_func = nn.MSELoss(reduction='mean')
    delta_label = None
    for epoch in range(epochs):
        features = []
        data_iterator = tqdm(train_dataloader, leave=True, unit="batch",
                             postfix={"epo": epoch, "acc": "%.4f" % (accuracy or 0.0),
                                      "lss": "%.8f" % 0.0, "dlb": "%.4f" % (delta_label or 0.0), },
                             disable=silent, )
        model.train()
        for index, batch in enumerate(data_iterator):
            if (isinstance(batch, tuple) or isinstance(batch, list)) and len(batch) == 2:
            ...
            if model.mode in ['IDEC', 'DCEC']:
                ...
                loss = clustering_loss + model.lambda_ * structure_loss

            else:
                loss = clustering_loss

            data_iterator.set_postfix(epo=epoch, acc="%.4f" % (accuracy or 0.0),
                                      lss="%.8f" % float(clustering_loss.item()),
            ...
            if update_freq is not None and index % update_freq == 0:
                loss_value = float(clustering_loss.item())
                data_iterator.set_postfix(epo=epoch,
                                          acc="%.4f" % (accuracy or 0.0),
                                          lss="%.8f" % loss_value, dlb="%.4f" % (delta_label or 0.0), )
                if update_callback is not None:
                    update_callback(accuracy, loss_value, delta_label)
        predicted, actual = predict(dataset, model, batch_size=evaluate_batch_size,
                                    collate_fn=collate_fn, silent=True, return_actual=True, cuda=cuda, )
        delta_label = (
        ...
        data_iterator.set_postfix(
            epo=epoch, acc="%.4f" % (accuracy or 0.0), lss="%.8f" % 0.0, dlb="%.4f" % (delta_label or 0.0), )
...
def predict(
    ...
    for batch in data_iterator:
        ...
        embeddings.append(model.get_embeddings(batch).detach().cpu())  # move to the CPU to prevent out of memory on the GPU
    if return_embeddings:
        return torch.cat(features).max(1)[1], torch.cat(actual).long(), torch.cat(embeddings)
